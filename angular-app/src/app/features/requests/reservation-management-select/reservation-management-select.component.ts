...
export class ReservationManagementSelectComponent {
    ...
    rawFilteredMaterials: MaterialInStockDTO[] = [];
    ...
    private materialsCache = new Map<number, {
        data: MaterialInStockDTO[],
        loadedAt: number
    }>();
    ...
    findMaterialsByContractReference(item: ReserveItemRequest) {
        ...
        const cache = this.materialsCache.get(item.contractReferenceItemId);

        const isCacheValid =
            cache &&
            (Date.now() - cache.loadedAt) < this.CACHE_TTL;

        if (isCacheValid) {

            this.rawFilteredMaterials = cache.data.sort((a, b) => {
        ...
        this.executionService
            .findMaterialsByContractReference(
            ...
            .subscribe({
                ...
                next: (response) => {
                    ...
                    this.rawFilteredMaterials = response.sort((a, b) => {
                        const depositOrder = a.depositName.localeCompare(b.depositName);
    ...
    get availableBrands(): { label: string; value: string }[] {
        const brandsSet = [...new Set(
            this.rawFilteredMaterials
                .map(m => m.materialBrand)
                .filter((b): b is string => !!b)
        )];

        const brands = brandsSet.map(b => ({label: b.toUpperCase(), value: b}));
        if (brands.length === 1) {
            this.selectedBrand = brands[0].value;
        }
    ...
    applyFilters() {
        this.filteredMaterials = this.rawFilteredMaterials.filter(m => {
            const inStock = !this.showOnlyInStock || Number(m.stockAvailable) > 0;
            const byBrand = !this.selectedBrand || m.materialBrand === this.selectedBrand;
            return inStock && byBrand;
        });

        setTimeout(() => {
            // Encontra o elemento que realmente tem a barra de rolagem.
            // Substitua '.meu-container-scroll' pela classe ou ID correto da sua div principal.
            // Se você usa o layout padrão do Angular, pode ser o elemento <main> ou um wrapper.
    ...
    private materialsHistoryCache = new Map<number, {
        data: any[],
        loadedAt: number
    }>();
    ...
    getStockHistory(item: ReserveItemRequest) {
        ...
        const cache = this.materialsHistoryCache.get(
            item.contractReferenceItemId
        );
        ...
        if (isCacheValid) {
            this.filteredMaterialHistory = cache.data;
            return;
        }
        ...
        this.requestService
            ...
            .subscribe({
                ...
                next: (response) => {
                    ...
                    this.materialsHistoryCache.set(
                        item.contractReferenceItemId,
                        {
                            data: response,
                            loadedAt: Date.now()
...
Obrigado.
    ...
    get getTooltipBrand(): string {
        const availableBrandsLength = this.availableBrands.length;

        if (availableBrandsLength === 0) {
            return 'Não há marcas disponíveis para o item selecionado';
        }

        if (availableBrandsLength === 1) {
            return 'O filtro foi desabilitado porque existe apenas uma marca disponível';
        }

        return '';
    }

    get getCurrentItem(): ReserveItemRequest | null {
        if (this.currentItemId === 0) return null;

        return this.reserve.items.find(i => i.contractItemId === this.currentItemId) ?? null;
    }
