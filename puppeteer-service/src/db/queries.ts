...
export const getConventionalMaintenancesByStreetId = async (maintenanceId: string, streetIds: string[]) => {
    ...
    const result = await db.oneOrNone(
      `
        WITH items_by_street AS (
          ...
          WHERE msi.maintenance_street_id IN ($1:csv)
        )
        ...
        SELECT
          ...
          (
            ...
            WHERE ms.maintenance_id = m.maintenance_id
              AND ms.maintenance_street_id IN ($1:csv)
          ) AS streets,
        ...
        WHERE m.maintenance_id = $2;
      `, [streetIds, maintenanceId]
    );
...
export const getLedMaintenancesByStreetId = async (maintenanceId: string, streetIds: string[]) => {
